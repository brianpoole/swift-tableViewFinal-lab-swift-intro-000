...
class PlanetTableViewController: UITableViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let dest = segue.destination as? FactTableViewController, let indexPath = tableView.indexPathForSelectedRow{
            dest.property = planets[indexPath.row]
        }
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let moon_count = planets[indexPath.row].numberOfMoons
        var moon_label = "Moons"
        if moon_count == 1{
            moon_label = "Moon"
        }
        cell.numberOfMoonsLabel.text = "\(moon_count) \(moon_label)"
